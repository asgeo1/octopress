...
""" Requirements: s3cmd, octopress, gzip & sass. 
...
def gzip_files():
    for root, dirs, files in os.walk("public/"): # Traverse the public/ directory
        for f in files:
            if os.path.splitext(f)[1] in ['.html', '.css', '.js', '.svg']:
...
def deploy_to_s3_bucket(bucket, dry_run=False):
    ...
    try: # Upload all uncompressed files
        command = "s3cmd sync -P --exclude '*.html' --exclude '*.js' --exclude '*.css' --exclude '*.svg' public/ " + bucket
        if dry_run:
    ...
    try: # Upload all compressed files and add an appropriate header
        command = "s3cmd sync -P --add-header='Content-Encoding: gzip' --exclude '*.*' --include '*.html' --include '*.js' --include '*.css' --include '*.svg' public/ " + bucket
        if dry_run:
            command_ls = command.split()
            command_ls.insert(-2, "--dry-run")
            command = ' '.join(command_ls)
    ...
    try: # Remove any files that have been deleted
        command = "s3cmd sync -P --delete-removed public/ " + bucket
        if dry_run:
            command_ls = command.split()
            command_ls.insert(-2, "--dry-run")
            command = ' '.join(command_ls)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Deploy a Octopress site to Amazon S3")
